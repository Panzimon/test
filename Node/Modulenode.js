...
//一个模块想要对外暴露变量（函数也是变量），
// 可以用module.exports = variable;，
// 一个模块要引用其他模块暴露的变量，
// 用var ref = require('module_name');
// 就拿到了引用模块的变量。
...
/*
 ...
 如果你在a.js中使用了全局变量s，
 那么，在b.js中也使用全局变量s，
 将造成冲突，b.js中对s赋值会改变a.js的运行逻辑。

 也就是说，
 JavaScript语言本身并没有一种模块机制来保证不同模块
 可以使用相同的变量名
 */
...
/*
 ...
 其实要实现“模块”这个功能，
 并不需要语法层面的支持。
 Node.js也并不会增加任何JavaScript语法。
 实现“模块”功能的奥妙就在于JavaScript是一种函数式编程语言，
 它支持闭包。如果我们把一段JavaScript代码用一个函数包装起来，
 ...
 */
//原来的全局变量s现在变成了匿名函数内部的局部变量。
// 如果Node.js继续加载其他模块，
// 这些模块中定义的“全局”变量s也互不干扰。
...
/*
 ...
 首先，
 Node会把整个待加载的hello.js文件放入一个包装函数load中执行
...
/*
 ...
 所以我们可以得出结论：直接对module.exports赋值，
 可以应对任何情况：
